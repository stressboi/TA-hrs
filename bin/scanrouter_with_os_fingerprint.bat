...
REM requires npcap to be installed due to fingerprinting - see the readme
REM -------------------------------------------
...
echo %date% %time% external_ip="%IP%" default_gateway="%DefaultGateway%" DNS="%DNS%" gateway_mac=%MAC% reporting_host="%COMPUTERNAME%" org="%ORG%" loc="%LOC%" network_category=%CATEGORY% >> "%SPLUNK_HOME%\etc\apps\TA-hrs\bin\monitor\%COMPUTERNAME%_default_gw_output.log"
...
if %CATEGORY%==Private (
     "%SPLUNK_HOME%\etc\apps\TA-hrs\bin\nmap\nmap.exe" -O -oX "%SPLUNK_HOME%\etc\apps\TA-hrs\bin\output\%COMPUTERNAME%_%DefaultGateway%.xml" -A %DefaultGateway%
     ) else (
     echo "Public network, no nmap scan will occur, exiting..."
     )